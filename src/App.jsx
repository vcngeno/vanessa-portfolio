...
function ContactForm() {
  ...
  const inp = (field) => ({
    background: "#13142a", border: `1px solid ${errors[field] ? "#f87171" : "#1e1f35"}`,
    borderRadius: 8, padding: ".75rem 1rem", color: "#e8e8f8",
  ...
  if (status === "success") return (
    <div style={{ background: "rgba(52,211,153,.1)", border: "1px solid rgba(52,211,153,.3)", borderRadius: 12, padding: "2rem", textAlign: "center" }}>
      ...
      <h3 style={{ color: "#34d399", marginBottom: ".5rem" }}>Message Sent!</h3>
      <p style={{ color: "#8888aa", fontSize: ".9rem" }}>Thanks for reaching out. Vanessa will get back to you soon.</p>
      <button onClick={() => setStatus("idle")} style={{ marginTop: "1rem", background: "transparent", border: "1px solid #1e1f35", borderRadius: 8, padding: ".6rem 1.25rem", color: "#e8e8f8", cursor: "pointer", fontSize: ".875rem" }}>Send Another</button>
    </div>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: ".85rem" }}>
      ...
      <button onClick={handleSubmit} disabled={status === "sending"}
        style={{ background: "linear-gradient(135deg,#a78bfa,#34d399)", border: "none", borderRadius: 8, padding: ".8rem", color: "#fff", fontWeight: 700, fontSize: ".95rem", cursor: status === "sending" ? "not-allowed" : "pointer", opacity: status === "sending" ? .7 : 1 }}>
...
Answer questions about Vanessa accurately and enthusiastically. Keep answers concise (2-4 sentences).`;
  ...
  return (
    <div style={{ background: "#13142a", border: "1px solid #1e1f35", borderRadius: 16, overflow: "hidden", display: "flex", flexDirection: "column", height: 420 }}>
      <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid #1e1f35", display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 10, height: 10, borderRadius: "50%", background: "#34d399", boxShadow: "0 0 8px #34d399" }} />
        <span style={{ fontSize: ".85rem", fontWeight: 600, color: "#e8e8f8" }}>AI Portfolio Assistant</span>
        <span style={{ marginLeft: "auto", fontSize: ".7rem", color: "#8888aa", background: "rgba(167,139,250,.1)", border: "1px solid rgba(167,139,250,.2)", padding: "2px 8px", borderRadius: 20 }}>Powered by Claude</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "1rem 1.25rem", display: "flex", flexDirection: "column", gap: 12 }}>
        {msgs.map((m, i) => (
          <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            <div style={{ maxWidth: "80%", padding: ".65rem 1rem", borderRadius: m.role === "user" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: m.role === "user" ? "#a78bfa" : "#0e0f1c", border: m.role === "assistant" ? "1px solid #1e1f35" : "none", fontSize: ".875rem", lineHeight: 1.6, color: "#e8e8f8" }}>{m.text}</div>
          </div>
        ))}
        {loading && (
          <div style={{ display: "flex" }}>
            <div style={{ background: "#0e0f1c", border: "1px solid #1e1f35", borderRadius: "16px 16px 16px 4px", padding: ".65rem 1rem" }}>
              <span style={{ color: "#8888aa", fontSize: ".85rem" }}>Thinking...</span>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      <div style={{ padding: ".75rem 1.25rem", borderTop: "1px solid #1e1f35", display: "flex", gap: 8 }}>
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Ask about Vanessa's experience..."
          style={{ flex: 1, background: "#0e0f1c", border: "1px solid #1e1f35", borderRadius: 8, padding: ".6rem 1rem", color: "#e8e8f8", fontSize: ".875rem", outline: "none" }} />
        <button onClick={send} disabled={loading} style={{ background: "#a78bfa", border: "none", borderRadius: 8, padding: ".6rem 1.1rem", color: "#fff", fontWeight: 700, cursor: "pointer", fontSize: ".875rem" }}>↑</button>
      </div>
    </div>
  );
}
...
export default function Portfolio() {
  ...
  return (
    <div style={{ background: "#07080f", color: "#e8e8f8", minHeight: "100vh", fontFamily: "'Segoe UI', system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      <nav style={{ position: "fixed", top: 0, width: "100%", zIndex: 100, backdropFilter: scrolled ? "blur(16px)" : "none", background: scrolled ? "rgba(7,8,15,.85)" : "transparent", borderBottom: scrolled ? "1px solid #1e1f35" : "none", padding: "1rem 2rem", display: "flex", justifyContent: "space-between", alignItems: "center", transition: ".3s" }}>
        <div style={{ fontWeight: 800, fontSize: "1.1rem", background: "linear-gradient(135deg,#a78bfa,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>VN.</div>
        <div style={{ display: "flex", gap: "1.75rem" }}>
          {["About","Skills","Projects","Experience","Contact"].map(n => (
            <a key={n} href={"#"+n.toLowerCase()} style={{ color: "#8888aa", textDecoration: "none", fontSize: ".875rem" }}
              onMouseEnter={e => e.target.style.color="#e8e8f8"} onMouseLeave={e => e.target.style.color="#8888aa"}>{n}</a>
          ))}
        </div>
      </nav>
      ...
      <section id="about" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "8rem 2rem 4rem", position: "relative", zIndex: 1 }}>
        <div style={{ maxWidth: 860, width: "100%" }}>
          <div style={{ display: "inline-block", background: "rgba(167,139,250,.12)", border: "1px solid rgba(167,139,250,.25)", color: "#a78bfa", padding: ".3rem 1rem", borderRadius: 20, fontSize: ".78rem", fontWeight: 600, letterSpacing: ".06em", marginBottom: "1.5rem" }}>✦ Open to Full-Time Opportunities · USA</div>
          <h1 style={{ fontSize: "clamp(2.8rem,6vw,5rem)", fontWeight: 900, lineHeight: 1.05, marginBottom: "1.25rem" }}>
            Vanessa<br />
            <span style={{ background: "linear-gradient(135deg,#a78bfa,#34d399,#60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Ngeno</span>
          </h1>
          <p style={{ fontSize: "1.15rem", color: "#8888aa", maxWidth: 580, lineHeight: 1.75, marginBottom: "1rem" }}>
            Data Scientist & Analytics Engineer who turns complex datasets into <strong style={{ color: "#e8e8f8" }}>evidence-based decisions</strong> — from correctional literacy systems to AI-powered tutoring platforms.
          </p>
          <p style={{ fontSize: ".9rem", color: "#8888aa", marginBottom: "2.5rem" }}>MS Data Analytics & Visualization · Yeshiva University &nbsp;|&nbsp; Ex-Safaricom BI Lead · Nairobi</p>
          <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginBottom: "4rem" }}>
            <a href="#projects" style={{ padding: ".75rem 1.75rem", background: "#a78bfa", color: "#fff", borderRadius: 8, fontWeight: 700, textDecoration: "none", fontSize: ".95rem" }}>View Projects</a>
            <a href="#contact" style={{ padding: ".75rem 1.75rem", background: "transparent", color: "#e8e8f8", border: "1px solid #1e1f35", borderRadius: 8, fontWeight: 600, textDecoration: "none", fontSize: ".95rem" }}>Get In Touch</a>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1.5rem", borderTop: "1px solid #1e1f35", paddingTop: "2.5rem" }}>
            {stats.map((s, i) => (
              <div key={i} style={{ textAlign: "center" }}>
                <div style={{ fontSize: "clamp(1.4rem,3vw,2rem)", fontWeight: 800, background: "linear-gradient(135deg,#a78bfa,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{s.num}</div>
                <div style={{ fontSize: ".72rem", color: "#8888aa", marginTop: 4 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding: "2rem 2rem 5rem", maxWidth: 860, margin: "0 auto", position: "relative", zIndex: 1 }} data-id="chat">
        <div style={fadeIn("chat")}>
          <div style={{ textAlign: "center", marginBottom: "2rem" }}>
            <div style={{ color: "#a78bfa", fontSize: ".78rem", fontWeight: 600, letterSpacing: ".1em", marginBottom: ".5rem" }}>POWERED BY CLAUDE API</div>
            <h2 style={{ fontSize: "clamp(1.6rem,3vw,2.4rem)", fontWeight: 800 }}>Ask My AI Assistant</h2>
            <p style={{ color: "#8888aa", marginTop: ".5rem", fontSize: ".9rem" }}>Ask anything about my background, skills, or projects</p>
          </div>
          <AIChat />
        </div>
      </section>
      ...
      <section id="skills" style={{ padding: "5rem 2rem", maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div data-id="skills-header" style={fadeIn("skills-header")}>
          <div style={{ color: "#a78bfa", fontSize: ".78rem", fontWeight: 600, letterSpacing: ".1em", marginBottom: ".75rem" }}>TOOLKIT</div>
          <h2 style={{ fontSize: "clamp(1.8rem,3vw,2.6rem)", fontWeight: 800, marginBottom: ".75rem" }}>Skills & <span style={{ background: "linear-gradient(135deg,#a78bfa,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Expertise</span></h2>
          <p style={{ color: "#8888aa", marginBottom: "2.5rem", fontSize: ".95rem" }}>End-to-end capabilities from data engineering to AI deployment.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(270px,1fr))", gap: "1.25rem" }}>
          {skills.map((s, i) => (
            <div key={i} data-id={"skill-"+i} style={{ ...fadeIn("skill-"+i, { transitionDelay: i*.08+"s" }), background: "#13142a", border: "1px solid #1e1f35", borderRadius: 14, padding: "1.5rem", cursor: "default", transition: "border-color .2s, transform .2s" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor="#a78bfa"; e.currentTarget.style.transform="translateY(-4px)"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor="#1e1f35"; e.currentTarget.style.transform="translateY(0)"; }}>
              <div style={{ marginBottom: ".75rem" }}>{SkillIcons[s.icon]("#a78bfa")}</div>
              <h3 style={{ fontSize: "1rem", marginBottom: ".5rem" }}>{s.name}</h3>
              <div style={{ display: "flex", flexWrap: "wrap", gap: ".35rem", marginTop: ".75rem" }}>
                {s.tags.map((t, j) => <span key={j} style={{ background: "rgba(167,139,250,.1)", border: "1px solid rgba(167,139,250,.18)", color: "#a78bfa", padding: ".18rem .55rem", borderRadius: 4, fontSize: ".72rem" }}>{t}</span>)}
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="projects" style={{ padding: "5rem 2rem", maxWidth: 1100, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div data-id="proj-header" style={fadeIn("proj-header")}>
          <div style={{ color: "#34d399", fontSize: ".78rem", fontWeight: 600, letterSpacing: ".1em", marginBottom: ".75rem" }}>PORTFOLIO</div>
          <h2 style={{ fontSize: "clamp(1.8rem,3vw,2.6rem)", fontWeight: 800, marginBottom: ".75rem" }}>Featured <span style={{ background: "linear-gradient(135deg,#a78bfa,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Projects</span></h2>
          <p style={{ color: "#8888aa", marginBottom: "2.5rem", fontSize: ".95rem" }}>Real-world impact across literacy, AI, and business intelligence.</p>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: "1.5rem" }}>
          {projects.map((p, i) => (
            <div key={i} data-id={"proj-"+i} style={{ ...fadeIn("proj-"+i, { transitionDelay: i*.1+"s" }), background: "#13142a", border: "1px solid #1e1f35", borderRadius: 16, padding: "1.75rem", position: "relative", overflow: "hidden", cursor: "default", transition: "border-color .2s, transform .2s" }}
              ...
              <div style={{ fontSize: ".78rem", color: "#8888aa", marginBottom: ".85rem" }}>{p.org}</div>
              <p style={{ fontSize: ".875rem", color: "#8888aa", lineHeight: 1.65, marginBottom: "1.25rem" }}>{p.desc}</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: ".35rem", marginBottom: "1.25rem" }}>
                {p.tags.map((t, j) => <span key={j} style={{ background: "rgba(255,255,255,.05)", border: "1px solid #1e1f35", color: "#8888aa", padding: ".18rem .55rem", borderRadius: 4, fontSize: ".72rem" }}>{t}</span>)}
              </div>
      ...
      <section id="experience" style={{ padding: "5rem 2rem", maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div data-id="exp-header" style={fadeIn("exp-header")}>
          <div style={{ color: "#60a5fa", fontSize: ".78rem", fontWeight: 600, letterSpacing: ".1em", marginBottom: ".75rem" }}>CAREER</div>
          <h2 style={{ fontSize: "clamp(1.8rem,3vw,2.6rem)", fontWeight: 800, marginBottom: ".75rem" }}>Work <span style={{ background: "linear-gradient(135deg,#a78bfa,#60a5fa)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Experience</span></h2>
          <p style={{ color: "#8888aa", marginBottom: "3rem", fontSize: ".95rem" }}>From Nairobi to New York — a decade of turning data into decisions.</p>
        </div>
        <div style={{ position: "relative", paddingLeft: "2rem" }}>
          <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 2, background: "linear-gradient(to bottom,#a78bfa,#34d399,#60a5fa)" }} />
          {experience.map((e, i) => (
            <div key={i} data-id={"exp-"+i} style={{ ...fadeIn("exp-"+i, { transitionDelay: i*.1+"s" }), position: "relative", marginBottom: "2.5rem" }}>
              <div style={{ position: "absolute", left: "-2.45rem", top: ".35rem", width: "1rem", height: "1rem", borderRadius: "50%", background: "#a78bfa", border: "2px solid #07080f", boxShadow: "0 0 10px #a78bfa" }} />
              <div style={{ fontSize: ".78rem", color: "#a78bfa", fontWeight: 600, marginBottom: ".3rem" }}>{e.period}</div>
              <h3 style={{ fontSize: "1.05rem", fontWeight: 700 }}>{e.role}</h3>
              <div style={{ fontSize: ".9rem", color: "#34d399", marginBottom: ".75rem" }}>{e.company}</div>
              <ul style={{ paddingLeft: "1.1rem" }}>
                {e.bullets.map((b, j) => <li key={j} style={{ fontSize: ".875rem", color: "#8888aa", lineHeight: 1.7, marginBottom: ".3rem" }}>{b}</li>)}
              </ul>
            </div>
          ))}
        </div>
      ...
      <section id="contact" style={{ padding: "5rem 2rem", maxWidth: 860, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div data-id="contact" style={fadeIn("contact")}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            <div style={{ color: "#a78bfa", fontSize: ".78rem", fontWeight: 600, letterSpacing: ".1em", marginBottom: ".75rem" }}>LETS TALK</div>
            <h2 style={{ fontSize: "clamp(1.8rem,3vw,2.6rem)", fontWeight: 800, marginBottom: ".75rem" }}>Get In <span style={{ background: "linear-gradient(135deg,#a78bfa,#34d399)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Touch</span></h2>
            <p style={{ color: "#8888aa", fontSize: ".95rem" }}>Open to full-time roles, freelance projects, and research collaborations.</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: ".85rem" }}>
              {[
                ...
                <a key={i} href={l.href} style={{ display: "flex", alignItems: "center", gap: ".75rem", background: "#13142a", border: "1px solid #1e1f35", borderRadius: 10, padding: ".85rem 1.1rem", color: "#e8e8f8", textDecoration: "none", fontSize: ".9rem", transition: "border-color .2s" }}
                  onMouseEnter={e => e.currentTarget.style.borderColor="#a78bfa"}
                  onMouseLeave={e => e.currentTarget.style.borderColor="#1e1f35"}>
      ...
      <footer style={{ textAlign: "center", padding: "2rem", color: "#8888aa", fontSize: ".8rem", borderTop: "1px solid #1e1f35", position: "relative", zIndex: 1 }}>
